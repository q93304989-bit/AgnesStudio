...
支持浅色 / 深色双主题，可跟随系统自动切换：

| 🌙 深色主题 | ☀️ 浅色主题 |
|:---:|:---:|
| ![深色主题](docs/ui-dark.png) | ![浅色主题](docs/ui-light.png) |
...
有用户对模型做了一轮**极限提示词测试**——把多个强约束堆在同一条提示词里，看看哪些能被精准执行、哪些会「穿模」。这恰好是理解扩散模型能力边界的绝佳素材。

**测试提示词（多个强约束叠加）**：

> 超写实摄影风格，微距构图，8k，电影级布光。
>
> 画面左侧是一只透明高脚杯，杯中盛有 2/3 深度的清水，水中插着**三朵完全相同的红玫瑰**，花茎在水中必须**精准交叉成 X 形**。画面右侧是一只成年男性的左手，拇指与食指捏着一张纯白便签纸，纸上用**黑色宋体清晰印刷着「AI极限」四个汉字**，要求笔画完整无缺，手指必须有真实的皮肤纹理与半月牙指甲高光。
>
> 背景是霓虹闪烁的模糊夜间都市，但天空中必须出现**两个大小完全一样的圆月**。画面右下角斜放着一枚金属硬币，币面雕刻着**逆时针旋转的复杂罗盘纹路**。光源来自正上方，要求在水面与杯底投射出**多棱角的清晰三角形折射阴影**。极浅景深，背景完全虚化，前景主体百分百锐利。

**实际生成结果**（`agnes-image-2.5-flash`，一次通过）：

![极限提示词测试：3朵玫瑰、双月、「AI极限」便签、罗盘硬币](docs/demo-limit-test.png)

**结果分析**：

- ✅ **精准命中**：3 朵玫瑰 + X 形交叉花茎、双圆月、「AI极限」中文印刷体（笔画完整）、捏纸手势与皮肤纹理、罗盘硬币、浅景深构图——全部到位；
- ⚠️ **仍会穿模**：透明玻璃杯两侧边缘与花瓣的**遮挡关系**偶尔出错（正面视角不穿，两侧视角会穿）；
- 💡 **为什么**：扩散模型学的是 **2D 像素概率分布，不是 3D 物理引擎**——它「见过」无数类似画面所以能画出文字和双月，但物体间的精确空间遮挡没有物理保证。

理解了这一点，你就理解了这类模型「什么擅长、什么不擅长」的底层逻辑。也欢迎提交你的边界测试结果到 [Issues](https://github.com/q93304989-bit/AgnesStudio/issues)，优质案例会收录在这里。
...
1. **实现 AI 自动化**（未来目标）：从「用户逐次点击生成」走向「AI 自主编排」——自动拆解需求、生成 / 自检 / 重试提示词、批量产出成图，人只负责最终挑选。
2. **AI 剧本生成**（下次更新）：输入一段故事设定或主题，AI 自动生成分镜提示词并逐帧出图，一键产出成套画面。
3. **UI 体验优化**：交互流程更顺滑、参数面板更直观、生成过程反馈更清晰。
